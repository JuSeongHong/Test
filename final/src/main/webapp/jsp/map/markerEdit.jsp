...
<body>
	...
	<form action="./markerEditAction" method="post" id="editMarker">
		<input type="hidden" name="mark_no" value="${markInfo.mark_no }">
		<div id="map"></div>	
		<div id="placeName">
			장 &nbsp;소&nbsp; 명 : <input type="text" id="Name" name="mark_name" value="${markInfo.mark_no }">				
		</div>
		...
		<div id="placeAddress">	
			...
			<input type="text" id="sample5_address" name="mark_address" value="${markInfo.mark_address }" >
				<input type="button" id="full" onclick="sample5_execDaumPostcode()"value="주소 검색">
		</div>
		<div id="placeTel">	
			전화번호 : <input type="text" id="Tel" name="mark_tel" value="${markInfo.mark_tel }">
		</div>	
		<div id="placeComment">	
			장소설명 : <br><textarea id="Comment" name="mark_comment" rows=10 style="resize:none; width:100%;">
			${markInfo.mark_comment }</textarea>
		</div>	
		<input type="hidden" id="mark_lat" name="mark_lat" value="${markInfo.mark_lat }"> 
		<input type="hidden" id="mark_lng" name="mark_lng" value="${markInfo.mark_lng }">
	...
	<script>
		...
		mapOption = {
			center : new daum.maps.LatLng(${markInfo.mark_lat }, ${markInfo.mark_lng }), // 지도의 중심좌표
			level : 4
		// 지도의 확대 레벨
		};
		...
		var marker = new daum.maps.Marker({
			position : new daum.maps.LatLng(${markInfo.mark_lat }, ${markInfo.mark_lng }),
			map : map
		});
