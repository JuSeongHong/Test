...
//수정폼, 체크박스 값 가져오기
$(function(){
	if('${petebean.pete_space}' == ('아파트')) {
		$("#pete_space2").attr('checked', true)
	} else if('${petebean.pete_space}' == ('주택')) {
		$("#pete_space1").attr('checked', true)
	}
	
	if('${petebean.pete_animals_kind}' == ('있음')) {
		$("#pete_animals_kind1").attr('checked', true)
	} else if ('${petebean.pete_animals_kind}' == ('없음')){
		$("#pete_animals_kind2").attr('checked', true)
	}
	
	if('${petebean.pete_walk}' == ('가능')) {
		$("#pete_walk1").attr('checked', true)
	} else if ('${petebean.pete_walk}' == ('불가능')){
		$("#pete_walk2").attr('checked', true)
	}
	
	if('${petebean.pete_family}' == ('있음')) {
		$("#pete_family1").attr('checked', true)
	} else if ('${petebean.pete_family}' == ('없음')){
		$("#pete_family2").attr('checked', true)
	}
})
...
<script src="./resources/js/pete_edit.js"></script>
...
<form method="post" action="./pete_edit_ok.nhn" enctype="multipart/form-data" onsubmit="return check()">
<input type="hidden" name="pete_board_num" value="${petebean.pete_board_num }">
<input type="hidden" name="page" value="${page }">
<table>
<c:set var="id" value="${petebean.pete_id }"/>
<c:set var="pid" value="${sessionScope.id}"/>
<c:if test="${id eq pid}">
	<div class="form-group-row">
			<h4 class="join_title col-sm-12">펫시터 집에 맡기기 수정</h4>				
	</div>
	
	<div class="form-group-row">
	<label for ="pete_id" class="col-sm-2 col-form-label">아이디</label>
		<div class="col-sm-10">
			<input name="pete_id" id="pete_id" type="text" size="10" value="${petebean.pete_id}" maxlength="30" class="form-control"> 								
		</div>
	</div>
	
	<div class="form-group-row">
	<label for ="pete_pass" class="col-sm-2 col-form-label">비밀번호</label>
		<div class="col-sm-10">
			<input name="pete_pass" id="pete_pass" type="password" size="10" maxlength="30" value="" class="form-control"> 								
		</div>
	</div>
	
	<div class="form-group-row">
	<label for ="pete_subject" class="col-sm-2 col-form-label">제목</label>
		<div class="col-sm-10">
			<input name="pete_subject" id="pete_subject" name="pete_subject" type="text" size="63"
			maxlength="100" value="${petebean.pete_subject}" class="form-control"> 							
		</div>
	</div>
	...
	<label for ="pete_content" class="col-sm-2 col-form-label">내용</label>
		<div class="col-sm-10">
			<textarea name="pete_content" id="pete_content" cols="67" rows="15" class="form-control">${petebean.pete_content}</textarea>							
		</div>
	</div>
	...
	<div class="form-group-row">	
		...
		<div class="col-sm-5">
			<input type="time" name="pete_time_in" id="pete_time_in"  value="${petebean.pete_time_in}" class="form-control">
		</div>
		<div class="col-sm-5">
			 <input type="time" name="pete_time_in2" id="pete_time_in2"  value="${petebean.pete_time_in2}" class="form-control"> 
		</div>
	</div>
	...
	<div class="form-group-row">	
		...
		<div class="col-sm-5">
			 <input type="time" name="pete_time_out" id="pete_time_out" value="${petebean.pete_time_out }" class="form-control"> 
		</div>
		<div class="col-sm-5">
			<input type="time" name="pete_time_out2" id="pete_time_out2" value="${petebean.pete_time_out2}" class="form-control">
		</div>
	</div>
	...
	<label for="pete_location" class="col-sm-2 col-form-label"></label>
		<div class="col-sm-10">
		<input type="text" id="pete_location" name="pete_location" placeholder="주소" size="50" value="${petebean.pete_location}" class="form-control">
		</div>
	</div>
	...
	<div>
		...
		<input type="text" value="">
			<input type="checkbox" id="pete_space" name="pete_space" value="${petebean.pete_space }" onclick="check_only(this)" >주택 
	...
	<div class="form-group-row">
		...
		<div class="col-sm-10">
			...
			<span id="filevalue">${petebean.pete_house_original }</span>&nbsp;<span id="close">
...
<c:if test="${id ne pid}">
<span>글쓴이가 아닙니다.</span>
<input type="reset" value="뒤로가기" onclick="history.go(-1)" class="form-control btn btn-info">
</c:if>	
</table>	
</form>
</div>
