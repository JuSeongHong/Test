...
public class PetsitterAction {
	...
	private String saveFolder = "C:\\Users\\nara\\Desktop\\20181219\\final\\src\\main\\webapp\\resources\\upload";
	...
	public String bbs_write_ok(
		...
		System.out.println("pete_space :" + space);
	...
	public ModelAndView pete_cont(
			...
			@RequestParam(value="page", defaultValue="1") int page,
			@RequestParam(value="state", defaultValue="cont") String state,
			HttpSession session
			) throws Exception {
		//번호를 기준으로 DB 내용을 가져옴
	...
	public ModelAndView pete_sitter (
		...
		//후기 갯수
		int listcount = psService.getEpilListCount(pete_id);
		...
		List<peteEpil> peteepil = psService.selectPeteEpil(m);
		List<peteEpil> epilSum = psService.selectEpilSum(pete_id);
		int sum = 0;
		for (peteEpil pete : epilSum) {
	...
	//후기 삭제
	@RequestMapping(value="pete_epil_del.nhn", method=RequestMethod.POST)
	@ResponseBody
	public void pete_epil_del(
			@RequestParam(value="pete_epil_no") int pete_epil_no,
			@RequestParam(value="join_epil_id") String join_epil_id,
			@RequestParam(value="pete_id") String pete_id
			) throws Exception {
		System.out.println("pete_id : " + pete_id);
		System.out.println("join_epil_id : " + join_epil_id);
		Map<String, Object> m = new HashMap<String, Object>();
		m.put("pete_epil_no", pete_epil_no);
		m.put("join_epil_id", join_epil_id);
		m.put("pete_id", pete_id);
		psService.deletePeteEpil(m);
	}
	
	//글쓴이 주소 지도
	@RequestMapping(value="pete_map.nhn")
	public String pete_map(
			@RequestParam("pete_location") String pete_location,
			Model model
			) throws Exception {
